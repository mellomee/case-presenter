...
function reorderItems(items, sourceIndex, destinationIndex) {
...
function reorderNodeBranch(nodes, parentLocalId, sourceIndex, destinationIndex) {
  if (parentLocalId === 'root') {
    return reorderItems(nodes, sourceIndex, destinationIndex);
  }
  ...
  return nodes.map((node) => {
    if (node.local_id === parentLocalId) {
      return {
        ...node,
        children: reorderItems(node.children || [], sourceIndex, destinationIndex),
      };
    }

    return {
      ...node,
      children: reorderNodeBranch(node.children || [], parentLocalId, sourceIndex, destinationIndex),
    };
  });
}

function parseDroppableId(droppableId) {
  const [pathKey, parentLocalId = 'root'] = droppableId.split('::');
  return { pathKey, parentLocalId };
}

function SortablePathChildren({ nodes, onPathChange, parties, proofs, defaultPartyId, depth, droppableId }) {
  return (
    <Droppable droppableId={droppableId} type="path-node">
      {(provided, snapshot) => (
        <div
        ...
        >
          {nodes.map((node, index) => (
            <Draggable key={node.local_id} draggableId={node.local_id} index={index}>
...
function PathQuestionNode({
  ...
  return (
    <div className={`${depth > 0 ? 'ml-4 pl-4 border-l border-slate-200' : ''}`}>
      <div className={`rounded-lg border border-slate-200 bg-white p-3 space-y-3 shadow-sm ${isDragging ? 'shadow-md ring-1 ring-blue-300' : ''}`}>
      ...
      <div className="mt-2">
        <SortablePathChildren
          ...
          defaultPartyId={defaultPartyId}
          depth={depth + 1}
          droppableId={`${node.admission_path}::${node.local_id}`}
        />
      </div>
    </div>
  );
...
function PathSection({ title, subtitle, pathKey, toneClass, questions, onChange, proofs, parties, defaultPartyId }) {
  return (
    <div className={`rounded-xl border p-4 ${toneClass}`}>
      ...
      <div className="mb-3 rounded-lg border border-dashed border-slate-300 bg-white/70 px-3 py-2 text-[11px] text-slate-500">
        Drag questions to reorder within this path. Follow-up questions can also be dragged within their own branch.
      </div>
...
export default function AdmissionPathQuestionsEditor({ value, onChange, proofs, parties, defaultPartyId }) {
  ...
  const handleDragEnd = (result) => {
    ...
    const { pathKey, parentLocalId } = parseDroppableId(source.droppableId);
    const reorderPath = (nodes) => reorderNodeBranch(nodes, parentLocalId, source.index, destination.index);

    if (pathKey === 'admitted') {
      onChange({ admitted: reorderPath(admitted), not_admitted: notAdmitted });
      return;
    }

    onChange({ admitted, not_admitted: reorderPath(notAdmitted) });
  };
