...
export default function ExamV2ImportDialog({
  open,
  onOpenChange,
  selectedParty = null,
  ...
  const downloadTemplate = () => {
    const templateRows = [
      ...
      [1, 'Proof', 'my new extract pages 1,3,5', 'Go to Page 3, please read the highlighted text', '', 1, '', '', 'some highlights'],
      [1, 'Proof', 'my new extract pages 1,3,5', 'What do you think?', '', 2, 'nah', '', ''],
      [1, 'Proof', 'my new extract pages 1,3,5', 'what is going on?', '', 3, '', '', ''],
      [2, 'Question Group', 'What human Factors', 'What does the document show?', '', 1, '', '', ''],
      [2, 'Question Group', 'What human Factors', 'Why does it matter?', 'What does the document show?', 2, '', 'Follow-up question', ''],
      ['', '', '', '', '', '', '', '', ''],
    ...
    try {
      ...
      rows.forEach((row, index) => {
        ...
        if (normalizedItemType === 'proof' && !matchedRootProof) {
          rowErrors.push(`Proof "${rootItemName}" was not found for this party/exam context.`);
        }
      ...
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto bg-slate-950 border-slate-800 text-white">
        ...
        {!preview ? (
          <div className="space-y-4">
            ...
            {error && (
              <div className="rounded-xl border border-red-900/50 bg-red-950/30 p-3 flex gap-2">
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <div className="rounded-xl border border-emerald-900/50 bg-emerald-950/30 p-3 text-center">
                <p className="text-2xl font-bold text-emerald-300">{validCount}</p>
                <p className="text-xs text-emerald-200">Exam Order Sections</p>
              </div>
              <div className="rounded-xl border border-blue-900/50 bg-blue-950/30 p-3 text-center">
                <p className="text-2xl font-bold text-blue-300">{totalQuestionCount}</p>
                <p className="text-xs text-blue-200">Questions</p>
              </div>
              <div className={`rounded-xl border p-3 text-center ${preview.errors.length > 0 ? 'border-red-900/50 bg-red-950/30' : 'border-slate-800 bg-slate-900/70'}`}>
                <p className={`text-2xl font-bold ${preview.errors.length > 0 ? 'text-red-300' : 'text-slate-300'}`}>{preview.errors.length}</p>
                <p className={`text-xs ${preview.errors.length > 0 ? 'text-red-200' : 'text-slate-400'}`}>Rows Skipped</p>
              </div>
            </div>

            <div className="max-h-72 overflow-y-auto space-y-3">
              {preview.rootItems.map((root, index) => (
                <div key={`${root.exam_order}-${root.root_item_name}-${index}`} className="rounded-xl border border-slate-800 bg-slate-900/70 p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <p className="text-xs uppercase tracking-[0.18em] text-slate-500">Exam Order {root.exam_order}</p>
                      <p className="mt-1 text-sm font-semibold text-white">{root.root_item_name}</p>
                      <p className="mt-1 text-xs text-slate-400">{root.item_type === 'proof' ? 'Proof' : 'Question Group'} · {root.question_rows.length} questions</p>
                    </div>
            ...
            {preview.errors.length > 0 && (
              <div className="rounded-xl border border-red-900/50 bg-red-950/20 p-4">
                <p className="text-sm font-semibold text-red-200">Skipped rows</p>
                <div className="mt-3 max-h-40 overflow-y-auto space-y-2">
                  {preview.errors.map((entry) => (
                    <div key={entry.rowNumber} className="text-xs text-red-200">
