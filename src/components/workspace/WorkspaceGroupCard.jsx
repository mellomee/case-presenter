    ...
    <div className="rounded-[28px] border border-slate-200 bg-white p-4 shadow-sm">
      <div className="grid gap-3 md:grid-cols-2">
        <input
          ...
          onBlur={() => onSaveGroup(group.id, { name })}
        ...
        <input
          ...
          onBlur={() => onSaveGroup(group.id, { node_label: nodeLabel })}
      ...
      <div className="mt-3 grid gap-3 md:grid-cols-[minmax(0,1fr)_220px]">
        <textarea
          ...
          onBlur={() => onSaveGroup(group.id, { why_it_matters: whyItMatters })}
        ...
        <select
          ...
          onChange={(event) => {
            ...
            onSaveGroup(group.id, { proof_id: event.target.value || null });
          }}
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        <button onClick={() => onAddQuestion(group)} className="rounded-full bg-blue-600 px-3 py-1.5 text-xs font-semibold text-white hover:bg-blue-700">
          Add Question
        </button>
        <button
          onClick={() => focusProof && onAddAdmissionBlock(group, focusProof)}
          disabled={!focusProof}
          className="rounded-full border border-slate-300 px-3 py-1.5 text-xs font-semibold text-slate-700 hover:bg-slate-50 disabled:text-slate-400"
        >
          Add Admission Block
        </button>
      </div>
