...
export default function VideoViewer({
  ...
  const clipSegment = isClip && proof?.video_clips?.length > 0 ? proof.video_clips[0] : null;
  const clipStart = clipSegment?.start ?? 0;
  const clipEnd = clipSegment?.end ?? null;
  ...
  const debouncedPush = useCallback(
    debounce((s) => onStateChange && onStateChange(s), 500),
    [onStateChange]
  );

  // Mirror sync state (viewer mode)
  useEffect(() => {
  ...
  const pushState = useCallback((overrides = {}) => {
    debouncedPush({
      playing,
      currentTime,
      volume,
