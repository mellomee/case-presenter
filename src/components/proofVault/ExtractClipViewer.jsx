  ...
  return (
    <div className="flex flex-col h-full bg-zinc-900">
      ...
      <div className="shrink-0 bg-zinc-800 border-b border-zinc-700 px-4 py-2 flex flex-wrap items-center gap-x-3 gap-y-1">
        ...
        {parentExtract && (
          <>
            <div className="flex items-center gap-1.5 text-xs">
              ...
              {parentExtract.extract_pages && (
                <span className="ml-1 bg-cyan-500/20 text-cyan-300 border border-cyan-500/30 text-[10px] px-1.5 py-0.5 rounded font-mono">
                  Pages {parentExtract.extract_pages}
                </span>
              )}
            </div>
            <span className="text-zinc-600 text-xs">›</span>
        ...
        {/* Exhibit number (right-aligned) */}
        {(proof.admitted_exhibit_num || proof.joint_exhibit_num || proof.draft_exhibit_num) && (
          <span className="ml-auto text-xs bg-amber-500/15 text-amber-400 border border-amber-500/25 px-2 py-0.5 rounded font-mono">
            Ex. {proof.admitted_exhibit_num || proof.joint_exhibit_num || proof.draft_exhibit_num}
          </span>
        )}
      </div>

      {/* PDF viewer — locked to clipped page, with highlights */}
