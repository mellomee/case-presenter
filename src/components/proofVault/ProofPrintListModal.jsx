...
export default function ProofPrintListModal({ open, onClose, proofs = [] }) {
  ...
  const rows = useMemo(() => {
    ...
    const buildStatusList = (status) => [
      ...topLevelExhibits.filter((proof) => proof.status === status),
      ...promotedExtracts.filter((proof) => proof.status === status),
    ];
