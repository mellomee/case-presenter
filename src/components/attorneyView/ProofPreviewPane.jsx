...
export default function ProofPreviewPane({ proof, juryState, onUpdateJury, onRuling, onClose }) {
  ...
  const renderPreview = () => {
    ...
    if (proof.proof_child_type === 'VideoClip') {
      return (
        <div className="video-clip-preview w-full h-full px-2 py-2 overflow-hidden">
          <VideoClipController
  ...
  return (
    <div className="flex flex-col h-full">
      <style>{`
        ...
        .video-clip-preview .relative.bg-slate-900.rounded-lg.overflow-hidden.aspect-video.border.border-slate-200 {
          aspect-ratio: auto;
          height: min(42vh, calc(100% - 108px));
          flex-shrink: 0;
        }

        .video-clip-preview .rounded-lg.border.border-slate-200.bg-slate-950.px-3.py-3.shadow-sm {
          padding: 0.5rem 0.75rem;
          flex-shrink: 0;
        }

        .video-clip-preview .rounded-lg.border.border-slate-200.bg-slate-950.px-3.py-3.shadow-sm .space-y-3 {
          gap: 0.5rem;
        }
      `}</style>
