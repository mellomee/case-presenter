...
Deno.serve(async (req) => {
  try {
    ...
    const applyOcr = Boolean(payload.applyOcr);
    ...
    if (!applyOcr && !addCoverPage && !addPageNumbers && !optimizePdf) {
    ...
    // Apply OCR only if requested and PDF is not already searchable
    let nextBytes = (applyOcr && !alreadySearchable) ? await runAdobeOcr(originalBytes) : originalBytes;

    // If specific pages are requested, extract only those pages
